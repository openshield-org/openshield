...
- Azure Firewall threat intelligence must be `AlertAndDeny`.
- An enabled Application Gateway WAF must use Prevention mode and export access, performance, and firewall diagnostic categories.
...
- Public Application Gateways must have an enabled `RateLimitRule` in the associated WAF policy.
