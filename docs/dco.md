...
reachable from that baseline are exempt, so release pull requests do not
retroactively reject project history created before the policy. New commits
remain subject to DCO even when they are added to a branch created before the
baseline.
