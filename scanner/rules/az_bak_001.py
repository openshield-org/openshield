...
def _unsafe(v):
 s=v.get("soft_delete_state"); d=v.get("soft_delete_retention_days"); return None if s is None or d is None else str(s).lower() not in {"enabled","alwayson"} or d < 35
def scan(azure_client, subscription_id): return scan_backup(azure_client, globals(), _unsafe, ["soft_delete_state","soft_delete_retention_days"])
