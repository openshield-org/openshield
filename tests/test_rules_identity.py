...
def _assignment(role_guid, principal_id, assign_id):
    return make_resource(
        id=(
            f"/subscriptions/{_SUB}/providers/"
            f"Microsoft.Authorization/roleAssignments/{assign_id}"
        ),
...
def test_idn_001_noncompliant_returns_one_finding(
    mock_azure,
    subscription_id,
):
...
def test_idn_002_compliant_policy_enforces_mfa_returns_no_findings(
    mock_azure,
    subscription_id,
):
    ...
    assert az_idn_002.scan(
        mock_azure,
        subscription_id,
    ) == []


def test_idn_002_noncompliant_no_policies_returns_one_finding(
    mock_azure,
    subscription_id,
):
...
def test_idn_003_compliant_restricted_invites_returns_no_findings(
    mock_azure,
    subscription_id,
    monkeypatch,
):
    ...
    assert az_idn_003.scan(
        mock_azure,
        subscription_id,
    ) == []


def test_idn_003_noncompliant_everyone_can_invite_returns_one_finding(
    mock_azure,
    subscription_id,
    monkeypatch,
):
...
def test_idn_004_compliant_role_has_pim_returns_no_findings(
    mock_azure,
    subscription_id,
    monkeypatch,
):
    ...
    assert az_idn_004.scan(
        mock_azure,
        subscription_id,
    ) == []


def test_idn_004_noncompliant_role_without_pim_returns_finding(
    mock_azure,
    subscription_id,
    monkeypatch,
):
...
def test_idn_005_compliant_member_user_returns_no_findings(
    mock_azure,
    subscription_id,
    monkeypatch,
):
    ...
    assert az_idn_005.scan(
        mock_azure,
        subscription_id,
    ) == []


def test_idn_005_noncompliant_guest_admin_returns_finding(
    mock_azure,
    subscription_id,
    monkeypatch,
):
...
def test_idn_006_compliant_fresh_secret_returns_no_findings(
    mock_azure,
    subscription_id,
    monkeypatch,
):
...
def test_idn_006_noncompliant_secret_no_expiry_returns_finding(
    mock_azure,
    subscription_id,
    monkeypatch,
):
...
def test_idn_007_compliant_user_with_mfa_returns_no_findings(
    mock_azure,
    subscription_id,
    monkeypatch,
):
    ...
    assert az_idn_007.scan(
        mock_azure,
        subscription_id,
    ) == []


def test_idn_007_noncompliant_user_without_mfa_returns_finding(
    mock_azure,
    subscription_id,
    monkeypatch,
):
...
def test_idn_007_disabled_user_without_mfa_returns_no_findings(
    mock_azure,
    subscription_id,
    monkeypatch,
):
...
def test_idn_008_compliant_specific_actions_returns_no_findings(
    mock_azure,
    subscription_id,
    monkeypatch,
):
    role = make_resource(
        ...
        assignable_scopes=[
            f"/subscriptions/{_SUB}"
        ],
    )

    _install_fake_auth_client(
        monkeypatch,
        [role],
    )

    assert az_idn_008.scan(
        mock_azure,
        subscription_id,
    ) == []


def test_idn_008_noncompliant_wildcard_action_returns_finding(
    mock_azure,
    subscription_id,
    monkeypatch,
):
...
def test_idn_009_compliant_alert_present_returns_no_findings(
    mock_azure,
    subscription_id,
    monkeypatch,
):
    ...
    alert = make_resource(
        enabled=True,
        condition=make_resource(
            all_of=[leaf]
        ),
    )

    _install_fake_monitor_client(
        monkeypatch,
        [alert],
    )

    assert az_idn_009.scan(
        mock_azure,
        subscription_id,
    ) == []


def test_idn_009_noncompliant_no_alert_returns_finding(
    mock_azure,
    subscription_id,
    monkeypatch,
):
